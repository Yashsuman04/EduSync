...
//   // Progress calculation after questions are available
...
//             style={{ width: `${progressPercent}%`, backgroundColor: "#332D56" }}
//             role="progressbar"
...
const Assessment = () => {
  ...
  useEffect(() => {
    const handlePopState = (e) => {
      ...
      setModalMessage("Back navigation is disabled during the assessment.");
  ...
  useEffect(() => {
    const disableRefresh = (e) => {
      if (
        ...
        e.preventDefault();
        setModalMessage("Refreshing is disabled during the assessment.");
        setModalVisible(true);
      }
    };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      e.preventDefault();
      e.returnValue = ""; // Modern browsers will show a default confirmation
    };
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e) => {
      // Show warning before unload (refresh, close, navigation)
      e.preventDefault();
...
//   // Set timer duration (e.g., 1 minute per question)
//   const TIMER_PER_QUESTION_SECONDS = 60; // change this as needed
...
//         // Initialize timer
//         setTimeLeft(
//           assessmentData.questions.length * TIMER_PER_QUESTION_SECONDS
//         );
//       } catch (err) {
//         console.error("Error fetching assessment:", err);
//         setError(err.message);
...
//   useEffect(() => {
//     if (timeLeft === null || submitted) return; // no timer if submitted or no time
...
//   // Auto-submit after time up popup shown for 3 seconds
//   useEffect(() => {
//     if (showTimeUpPopup) {
...
//       // No validation for unanswered questions — null answers accepted
...
//       const requestBody = { submission }; // <-- wrap in submission
...
//             "Content-Type": "application/json",
//           },
//           body: JSON.stringify(requestBody), // <-- use wrapped object
//         }
//       );
